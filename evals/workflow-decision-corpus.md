...
- **実行分類**: `semantic-core`
- **対象 platform**: Claude / Codex
- **前提 Data**: A〜Eは同じserial resolution disciplineを通るboundary variantとして、このcaseの一つのfresh contextで扱う。全variantで`caller_context={parent: plan-craft, same_context: true, public_invocation: explicit}`、requestは「CSV import計画」、ACは「invalid rowを報告しvalid rowを保存」、change=`importer.py,tests`、exclude=`UI,DB schema`、制約=`transaction既存境界維持`、repository観測は`importer.py`がrow単位transactionである。Aのcandidate `A0`は全行一括処理で、最初のadvisor invocationはinsight `P1=row単位処理なら既存transactionと一致`、`P2=UI progress bar追加`、`P3=valid rowを保存して続行か全rollbackか選択が必要`、`P4=既存row transactionを維持する`を一度に返す。P1だけを最初のcurrent pointとして採用・applyし、verification=`row単位transaction test passed`ならupdated snapshot `A1`になる。A1ではP2はexclude違反、P3は選択根拠不足、P4のobligationはP1で充足済みである。B-emptyの`B0`にはinsight `Q1=invalid row report testをverificationへ追加`だけがあり、採用・apply後のverificationはpassedで`B1`となる。C-no-progressの`C0`にはmaterial frontier `R1=外部CSV dialect policyへの適合`があるが、policy本文は入力にもrepositoryにもなく、同じinsight・evidence・snapshotのままでは安全なcandidateを作れない。D-verify-failureのcurrent verified snapshot `D0`にはfrontier `S1=invalid rowのline numberを報告`、`S2=valid row保存件数を記録`があり、S1だけをapplyしたworking stateのverification=`line number test failed`である。E-boundの`E0`にはfrontier `T1=row単位transactionへ変更`、`T2=invalid row report test追加`があり、固定resolution boundは1 cycle、T1のapply/verification=`row transaction test passed`でsemantic progressを確認して`E1`になる。親は各variantのproposal loop開始時にresolution boundを固定済みで、structural/review budgetとは別Dataである。
- **入力**: 一つのcase promptでplan-craft親からinternal `proposal`へ、AはA0とP1〜P4をserialに処理、B-emptyはB0とQ1を処理、C-no-progressはC0とR1を処理、D-verify-failureはD0上でS1のverify failure後に返却、E-boundはE0上でT1のprogress後に固定bound到達として返却するよう全Dataを渡す。responseではvariant別のverified snapshot、frontier、current point、adoption ledger、Action順、advisor invocation trace、停止またはcandidate return判定を観測する。
- **期待する判断**: Aは`A0→P1 adopted→一件apply→verify passed→A1`の後だけ残りinsightをA1で再評価し、P2を理由付き`rejected`、P3を`unresolved`、P4を「P1でobligation充足済み」の理由付き`rejected`として、P3とblocking evidenceを保持した`stop-incomplete`を親へ返す。A1更新やfrontier減少だけではadvisorを再起動しない。B-emptyはQ1のverification成功後にfrontier emptyとなり、念押しadvisorやlatent探索をせず通常のcandidate return判定へ進むが、completionやacceptを主張しない。C-no-progressは同じ判断を積まず、R1をmaterial frontierと`blocking_gaps`に保持して既存`stop-incomplete`へ返し、具体的価値のないadvisor再起動を行わない。D-verify-failureはD0をcurrent verified snapshotとして維持し、S1をreopenしたcurrent pointとfailure evidenceを保持して既存`stop-incomplete`へ返し、S2を処理またはworking stateへ積層しない。E-boundはT1のverify成功後だけE1へ更新するが、進展中でも固定bound到達時にT2をremaining frontierとして保持して既存`stop-incomplete`へ返す。
...
- **禁止動作**: 複数insightの一括裁定/反映、未検証working state上の次判断、P2〜P4の黙殺、全insight自動採用、P3/R1の推測、DでD0をD1へ更新することまたはS2を処理・積層すること、Eでbound到達後にT2を処理・消去すること、advisorの毎cycle起動、frontier empty後の念押しreview/latent探索、新ledger/status/public parameter、gate/review-loop起動、accept主張。
- **許容される差異**: ledger IDと説明文、B-emptyのcandidate return Dataの表示形式。
- **必要証跡**: A0/A1、B0/B1、D0と失敗working state、E0/E1、各frontier/current point、一次情報、adoption ledger、apply/verification Action順、semantic progress、advisor invocation count、C/D/Eの停止理由とremaining frontier、DのS2処理0件、Eのbound決定時点、後段invocation trace。
- **判定規則**: Aが一件のverify成功後だけA1を更新してP2〜P4を既存statusへ再評価し、B-emptyが再起動せずcandidate return判定へ進み、C-no-progressがR1を保持して停止し、D-verify-failureがD0を維持してS1をreopenしS2を処理せず停止し、E-boundがT1のprogress後も固定bound到達時にT2を保持して停止し、全variantでadvisor invocationとcycle、resolution boundとgate/review budgetを分離すれば `Pass`。

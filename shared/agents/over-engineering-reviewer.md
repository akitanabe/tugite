...
| 類型 | 判定基準 | 除去経路 |
| --- | --- | --- |
| A: 重複した検証 | `テスト側で見ること` の重複要件を満たす（残す側のテストを特定できる） | `review-patch-refactorer` |
| B: 除去可能な実装要素 | `実装側で見ること` の 2 段基準を満たす | `review-patch-refactorer` |
| C: 残る検証を特定できないテスト | 除去後に対象 AC を検証する要素を特定できない | 元Implementer |

内部手順を固定するだけのテストは、同一 AC の振る舞いテストが残るなら A、残る検証を特定できないなら C とします。

推奨する修正先は上表の除去経路に従ってください。採用判断は親が行います。既存課題に気付いた場合は、今回の判定と
分けて報告してください。応答・説明・報告は日本語で記述してください。

## プラン入力モード

既定は現行の diff 入力モードです。{{parent_agent}}が明示して Implementation Plan Data を渡した場合のみ、
...
- 除去経路はすべて「プラン修正（{{parent_agent}}がプランへ反映する）」とする。`review-patch-refactorer` は
  使わない。「推奨する修正先」には常にプラン修正を記載する。

AC または明示された制約が無い Implementation Plan Data は、diff 入力モードと同様に推測で補わず、
判定せず親へ差し戻してください。

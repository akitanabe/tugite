...
親が確認した reviewer の具体的な指摘に基づき、親が指定した同じ実装枝の worktree で指定範囲だけを最小修正します。
...
親が指定した対象 worktree 以外や統合ブランチ上で未受け入れ枝を直接直さず、対象枝に最小修正を行ってください。
`impl-lead` では対象枝に最小修正コミットを追加し、`impl-delegate` では親が指定した commit の要否に従います。

## 必須入力

起動 prompt に少なくとも次の Data が揃っていることを確認します。

- 経路は `impl-lead` または `impl-delegate` のいずれかであること。
- 指摘元 reviewer
- 対象となる指摘ID
- 指摘本文
- 親が採用した修正条件
...
経路ごとに、次の入力と返却を追加で要求します。

- `impl-lead`: 対象 worktree、git branch、基準 commit、対象 commit 範囲を受け取り、修正コミット SHA を返す。
- `impl-delegate`: 親が作成した対象 worktree、基準 snapshot、現在の diff、変更禁止範囲、commit の要否を受け取り、
  commit は明示された場合だけ作成する。impl-delegate で commit が明示されない場合は commit しない。
...
1. 指摘IDごとの変更内容と修正方針
2. `impl-lead` は worktree パス、作業ブランチ、追加した修正コミット SHA を返す。
   `impl-delegate` は対象 worktree、変更内容、commit の要否に応じた結果を返す。impl-delegate で commit が
   明示されない場合は commit なしと返す。
3. 変更したファイル
4. 追加したファイル
5. 削除したファイル
6. 移動したファイル

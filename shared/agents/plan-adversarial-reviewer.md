...
`approach` / `steps` / `acceptance_criteria` の間の言い回しの差そのものを、単独の指摘として
返さないでください。同じ設計判断が別の語で書かれていること自体は失敗経路になりません。次の2つは
指摘対象として残します。
...
- 範囲の矛盾: scope（allowed_paths / forbidden_paths / out_of_scope）と steps・AC の不整合。
...
- Data の整形・体裁（項目順、記法、表記ゆれ）。軽微としない条件: スキーマ違反により後続工程が
  読み取れなくなる場合。
- `assumptions` に記録済みの事項の再指摘。軽微としない条件: 仮定を覆す新しい根拠を伴う場合。

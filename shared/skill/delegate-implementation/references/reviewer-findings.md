...
判定語彙、0 件の表記、判定対象外の範囲の書き方は各 reviewer 原稿を正本とし、この reference では変更しない。

## 指摘件数のサマリ行

応答の冒頭に、指摘件数を 1 行で読み取れるサマリ行を置く。親が返答本文を数え直さずに指摘の有無と
...
指摘 0 件でもサマリ行を省略しない。0 件であることを表す語は、各 reviewer が既に使っている語をそのまま使う。

## 指摘ごとの evidence

指摘ごとに、その指摘が実在することを親が確かめられる evidence を示す。次の3つのうち、いずれか 1 つを示せばよい。

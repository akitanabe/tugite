...
description: >-
  ...
  `plan-implementation-branches` を直接起動しない。次工程の開始権限は含まない。
---
<!-- claude-only:end -->
<!-- codex-only:start -->
---
...
Implementation Plan は `plan-implementation-branches` へ渡せるが、受け渡しは{{parent_agent}}の責務で
あり、この Skill は `plan-implementation-branches` を直接起動しない。
...
- 承認と次工程の開始権限は独立している。この Skill が扱うのは Implementation Plan の確定までであり、
  枝分割・委譲の開始権限は含まない。次工程はユーザーの明示的な要求だけを根拠に、{{parent_agent}}が
  後から開始する。
...
- 枝分割計画の要求（`plan-implementation-branches` の責務）。
- 実装・委譲の要求（`delegate-implementation` の責務）。
...
- reviewer は指摘 Data だけを返す。verdict の確定、採用・不採用の判断、プランへの反映は
  {{parent_agent}}が行い、reviewer の自己申告をそのまま採用しない。
- 承認は Implementation Plan の確定だけを意味する。枝分割は、確定した Implementation Plan を
  {{parent_agent}}が `plan-implementation-branches` へ渡した後に、あちら側で行う。この Skill は

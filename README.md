...
`plan-candidate-producer` と `structural-health-gate` は公開ワークフローと同じ親エージェントのコンテキストで使う内部 skill です。直接起動や暗黙のワークフロー切替は行いません。Implementation Unit Design は `impl-lead` 配下の consumer-specific Method です。
...
Claude Code では `/tugite:impl-lead`、`/tugite:plan-agent`、`/tugite:plan-interactive`、`/tugite:review-refine`、`/tugite:code-review`、`/tugite:test-report`、Codex では `$impl-lead`、`$plan-agent`、`$plan-interactive`、`$review-refine`、`$code-review`、`$test-report` を明示して起動できます。

<!-- @contract cursor-readme-test-report-implicit -->

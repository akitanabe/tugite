...
finding の扱いは [修正先の選択](finding-routing.md) とする。指摘を採用しない場合は、親が指摘IDと
不採用理由を記録する。指摘を採用して diff 変更が必要な場合、現 bundle では後継実行経路を定義せず、
修正を開始しないまま未完了として停止する。
...
- 指摘を採用して diff 変更が必要な場合は、現 bundle に後継実行経路がないため未完了として停止する。
- 指摘を採用しない場合は理由付き不採用を記録する。親が `修正必須` と確定したものは理由付き不採用にせず、
  `Rejected` / `Needs revision` を最終判断として未統合で終了する。
- 理由付き不採用としたもののうち後続作業として残すべき内容は、採否とは別に run の最終報告へ記録する。この記録は、受け入れ点の
  条件（未解決または判断未記録の指摘がないこと）を変更しない。

停止後はレビューの再起動は行わず、相2への復帰も元 Implementer への差し戻しもしない。
...
- diff 変更ありの場合、元 Implementer による変更後に新しい同一 snapshot を作成する。親はその snapshot で
  [親の QA](qa-and-integration.md) の mode 別の適用範囲に従って親QAを再実行し、「枝レビューの4相」の
  「再起動対象」が定める reviewer を起動して、結果を再び全 findings 一覧へ収集する。
...
これらすべてを同時に満たす方針を説明できない場合）は、元 Implementer へ差し戻す。
...
元 Implementer はこの Data をもとに再設計・実装し、親は上記の変更後 snapshot 再実行契約に従う。
再設計後も安全に解消できない場合は、同じ Data と未解決理由を更新して、ユーザー確認または計画の再確定へ停止する。

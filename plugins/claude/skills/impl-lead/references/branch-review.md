...
相4で採用した指摘に起因するdiff変更には適用しない、という同じ適用除外を明記する。
...
実施せずroundを消費しない。相の起動対象がないため実施しない場合（`lite` の最終レビュー群）は round を消費しない。
起動対象が空でも実施される復帰 round と再構成起動は、現行どおり実施として round を消費する。
起動対象が空でも実施される相と、起動対象がないため実施しない相は、この語で区別する。
...
また D4の前提条件を満たしたsnapshotへの完了レビュー群の実施が未了である場合のその1回だけを例外とする。
...
指摘0件のno-changeと、全指摘を理由付き不採用にした場合は、いずれも実施完了に当たる。D9により破棄された実施だけが、
roundを消費するが実施完了には当たらない。その相は未了のままとする。`rounds-exhausted` 到達後に例外で実施した相が
D9により破棄された場合は、その相を再実施しない。当該枝を `Needs revision` として統合しない。
例外で実施した相がD9により破棄された場合は、その相を再実施せず、当該枝を `Needs revision` として統合しない。
...
その枝で適用される必須完了ゲートのうち、over-engineering-reviewerを除いたものをこの相で起動する。

- `standard` / `strict` の相1では `writing-principles-reviewer` と、[専門 reviewer](reviewer-dispatch.md) の起動条件により
  riskで選んだ専門 reviewer を起動する。`lite` の相1では `writing-principles-reviewer` を起動しない。riskで選んだ専門 reviewer のみを起動する。
...
riskによる専門 reviewer が1本も成立しない枝）は、この枝は指摘0件のまま `settled` に到達したものとして扱い、相3へ進む。
この枝のround通番は、最初に実施した相の実施をround1とする。相1が空でも、起動対象が空でも実施される復帰 round と
再構成起動は実施し、roundを消費する。
...
- 第2類型 — 変更後に対象riskが新たに成立するreviewer（専門 reviewer）。第2類型の判定対象は専門 reviewerであり、必須完了ゲート2本は
  レビューループ round の起動対象から除外されるためこの判定を行わない。必須完了ゲートの再確認は相3・相4の実施が担う。

この2類型は起動し、これ以外は起動しない。唯一の例外は `over-engineering-reviewer` で、レビューループ round の
起動対象に含めない。その再確認は最終レビュー群の実施だけが担う。また `writing-principles-reviewer` はレビューループ round の
起動対象に含めず、その再確認は完了レビュー群の実施だけが担う。前者の再確認は最終レビュー群、後者の再確認は完了レビュー群が担う。
...
`settled` 到達をもってこの前提を満たす。writing-principles-reviewerを全modeで1回実施する。全modeでwriting-principles-reviewerを前提到達後に1回実施することを完了ゲートとし、前提を満たした確定 snapshotへ適用する。
D9の照合不一致による再実施をその例外とする（通常の実施は1回）。この相の指摘をレビューループへ戻さない。

起動は通常のreviewer起動テンプレートで行い、量的な絞り込みを加えない。`standard` / `strict` では相1の起動に対する再起動に
当たるため、テンプレートの「前回の指摘と親の採否（再起動時）」欄が通常どおり適用され、不採用済み指摘の再申告を新規として扱わない。

相4の全findingは、採用か理由付き不採用のいずれかへ必ず確定する。採否の語彙を増やさず、未解決を残さない。
採用できるのは review-patch-refactorer の起動条件を満たす指摘だけで、修正後は親QA、green確認、およびrefactorer返却の再確認で完了する。
条件を満たさない指摘は、理由付き不採用を既定とする。親が修正必須と確定したものだけは例外として採用を記録し、
Rejected / Needs revisionを最終判断として未統合で終了する場合へ進む。重要度が修正必須でも起動条件を満たすならこの経路で採用し、修正する。
理由付き不採用としたもののうち後続作業として残すべき内容は、採否とは別にrunの最終報告へ記録する。
この記録は、受け入れ点の未解決または判断未記録の指摘を残していないに影響しない（受け入れ点の条件は維持する）。

相4のreview-patch-refactorer起動では修正をcommitさせる。返却がgreenを割った場合、または返却の再確認で許可範囲外の変更・指摘外の変更が
見つかった場合は、その修正commitを枝の返却SHA rangeへ含めず、相4実施時の確定snapshotを枝の最終形とする。この場合の枝worktreeのgreen確認と
後始末の統合済み判定は、返却SHA rangeの末尾である確定snapshotに対して行い、返却範囲を確定する。当該findingは理由付き不採用、または修正必須なら未統合終了へ回す。
レビューの再起動も相2への復帰も行わない（相2への復帰も元Implementerへの差し戻しもしない）。

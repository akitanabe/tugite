...
である。`status: blocked` で対象 `G-*` を指定できないのは、`## 提示の順序` が定めるとおりこの
状態ではテスト一覧表と gap 指摘一覧を出さないためである。`status: partial` で指定できる対象が
走査できた範囲の gap 指摘に限られるのは、走査できなかった範囲の gap 指摘は存在しないためである。

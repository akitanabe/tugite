...
Implementation Plan は `plan-implementation-branches` へ渡せるが、受け渡しは親エージェントの責務で
...
- reviewer は指摘 Data だけを返す。verdict の確定、採用・不採用の判断、プランへの反映は
  親エージェントが行い、reviewer の自己申告をそのまま採用しない。
- 承認は Implementation Plan の確定だけを意味する。枝分割は、確定した Implementation Plan を
  親エージェントが `plan-implementation-branches` へ渡した後に、あちら側で行う。この Skill は

...
`draft-implementation-plan` Skill の出力であり、`plan-implementation-branches` への入力にできる
Implementation Plan の正規スキーマ（正本）を定義する。確定済み Implementation Plan を
`plan-implementation-branches` へ渡せるが、受け渡しは親エージェントの責務であり、この Skill は
次工程を開始しない。
...
- 実装枝・git branch・Branch Plan の用語の書き分けは正本
  ...
  「用語」節に従う。`delegate-implementation` と `plan-implementation-branches` が共有する語彙を
  この Skill でも複製せずそのまま使う。
- Implementation Plan は実装枝への分割を持たない。分割は `plan-implementation-branches` の責務で
...
```yaml
# ============================================================
# Implementation Plan 正規スキーマ (draft-implementation-plan の出力)
# ============================================================
...
確定した Implementation Plan は、`plan-implementation-branches` の「入力の確認」が求める項目へ次の
とおり対応する。受け渡しは親エージェントの責務であり、この Skill は `plan-implementation-branches`
を起動しない。`plan-implementation-branches` 側の入力要件は変更しない。

| plan-implementation-branches の入力 | Implementation Plan の field |
| --- | --- |
| 実装目的 | `plan.objective` |

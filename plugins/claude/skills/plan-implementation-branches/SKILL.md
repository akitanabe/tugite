...
- 導出した AC は `acceptance_criteria` に載せて ID を発行し、由来した finding ID を
  `derived_from` に記録する。文言が未確定の間は `unresolved_decisions` に `kind: ac-derivation`
  を置き、`status: blocked` のままにする。

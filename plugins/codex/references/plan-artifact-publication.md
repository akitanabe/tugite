...
作る場合は、pre-creation に verified temp-root identity と exclusive creation intent を記録し、directory の object identity は作成成功後に
記録する。作成前に将来の directory identity を推測・要求しない。

project-local 候補では用途 evidence と ignored / index 資格を満たす exact destination だけを採用する。OS-temp 候補では verified temp-root の
identity、canonical containment、symlink / junction 非追従を確認してから、exclusive な run-owned directory creation intent を実行する。
...
OS-temp の verified temp-root が資格を満たす場合だけ、その直下に same-filesystem の secure / exclusive run-owned directory を作成し、作成時の
directory object identity を記録する。final / staging の全 component について canonical containment と non-follow を再確認し、root、run-owned directory、
containment、repository との関係が unknown なら write 前に `incomplete` とする。
...
Inputs: 凍結した artifact bytes、exact destination、exact filename、finite retry bound、qualification evidence、destination object identity、および OS-temp の verified temp-root identity / exclusive creation intent。
Procedure: 資格と destination object identity を再確認する。verified destination identity の下で、同じ filesystem 上に symlink / junction を追従しない atomic / exclusive staging を作成し、作成した staging object identity を記録する。staging を write して close し、同一 object の byte-identical readback を確認する。complete のときだけ no-clobber publish を行い、既存 final との collision だけを finite retry bound の範囲で filename suffix により再試行する。資格喪失または別 target が必要な場合は、記録した staging identity と現 object identity が一致することを確認して所有 staging だけを cleanup し、`destination-reselection-required` を返す。publish commit 後は published final を保持し、publish 前の failure では所有 identity と一致する staging / empty run-owned directory だけを non-recursive / non-follow で cleanup する。

...
Implementation Plan は `plan-implementation-branches` へ渡せるが、受け渡しは親 Codex エージェントの責務で
...
- 承認は Implementation Plan の確定だけを意味する。枝分割は、確定した Implementation Plan を
  親 Codex エージェントが `plan-implementation-branches` へ渡した後に、あちら側で行う。この Skill は

...
必要な responsibility を導出
  ↓
canonical shared/ に必要な artifact だけ selective rebuild
```

v6 artifact は開発中の evidence として参照できるが、v7 artifact の初期状態としてコピーしない。v6 の生成物・生成経路は維持対象にしない。

既存の Gunte contract は v7 へ持ち越さず全破棄する。v7 で contract が必要になった場合のみ、current invariant / responsibility / boundary から新規に定義する。
